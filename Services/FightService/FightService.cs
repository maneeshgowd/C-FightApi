...
namespace net_7.Services.FightService
{
    public class FightService : IFightService
    {
        ...
        public async Task<ServiceResponse<FightResultDto>> Fight(FightRequestDto request)
        {
            ...
            try
            {
                var character = await _context.Characters.Include(c => c.Weapon).Include(c => c.Skills).Where(c => request.CharacterIds.Contains(c.Id)).ToListAsync();
                ...
                while (!defeated)
                {
                    foreach (var attacker in character)
                    {
                        ...
                        if (usedWeapon && attacker.Weapon is not null)
                        {
                            attackUsed = attacker.Weapon.Name;
                            damage = DoWeaponAttack(attacker, opponent);
                        }
                        else if (!usedWeapon && attacker.Skills is not null)
                        {
                        ...
                        if (opponent.HitPoints <= 0)
                        {
                            ...
                            response.Data.Log.Add($"{opponent.Name} wins with {attacker.HitPoints} HP left!");
                            break;
                        }
                    }
                }
        ...
        public async Task<ServiceResponse<AttackResultDto>> SkillAttack(SkillAttackDto request)
        {
            ...
            try
            {
                ...
                if (skill is null)
                {
                    throw new Exception($"{attacker.Name} dosen't know that skill!");
                }
